...
This module creates an AWS Elastic Container Registry (ECR) collection with comprehensive repository management features. It provisions ECR repositories with configurable lifecycle policies, repository access policies, image scanning, encryption settings, and optional replication configuration across AWS regions. The module supports both individual repository creation and bulk repository management for container image storage and distribution.
...
AWS credentials used for authentication should have below privileges:
...
**Pre-requisites**
Before running this target it is important to ensure that, developer has created files mentioned below on local workstation under root directory of git repository that contains code for primitives/segments. Note that these files are AWS specific.
...
| <a name="input_force_delete"></a> [force\_delete](#input\_force\_delete) | Allow deletion of ECR repositories even when they contain images (use with caution) | `bool` | `false` | no |
...
| <a name="input_image_names"></a> [image\_names](#input\_image\_names) | List of Docker image names to create ECR repositories for. Each name becomes a repository name in ECR | `list(string)` | `[]` | no |
...
| <a name="input_max_image_count"></a> [max\_image\_count](#input\_max\_image\_count) | Maximum number of Docker image versions to retain in ECR repositories via lifecycle policy | `number` | `500` | no |
...
| <a name="input_prefixes_pull_through_repositories"></a> [prefixes\_pull\_through\_repositories](#input\_prefixes\_pull\_through\_repositories) | List of repository prefixes to grant pull-through cache access for upstream registries | `list(string)` | `[]` | no |
...
| <a name="input_principals_pull_through_access"></a> [principals\_pull\_through\_access](#input\_principals\_pull\_through\_access) | Principal ARNs to provide with pull-through cache access to the ECR | `list(string)` | `[]` | no |
...
| <a name="input_protected_tags"></a> [protected\_tags](#input\_protected\_tags) | List of image tag prefixes to protect from deletion via lifecycle policies (e.g., `["prod", "release"]`) | `list(string)` | `[]` | no |
...
| <a name="input_scan_images_on_push"></a> [scan\_images\_on\_push](#input\_scan\_images\_on\_push) | Enable automatic vulnerability scanning of container images when pushed to repository | `bool` | `true` | no |
